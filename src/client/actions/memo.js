...
/* MEMO EIDT */
export function memoEditRequest(id, index, content){
  return (dispatch) => {
    dispatch(memoEdit());

    return axios.put('/api/memo/' + id, {content}).then((response)=>{
...
/* MEMO POST */
export function memoPostRequest(content){
  return (dispatch) => {
    dispatch(memoPost());

    return axios.post('/api/memo', {content})
    .then( response => {
